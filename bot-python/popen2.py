...
def generate(i):
    ...
    if protocol == 6:
        stdin2, stdout2, stderr2 = p2.exec_command(f"iperf -s -p {value} -i 1")
        stdin1, stdout1, stderr1 = p1.exec_command(f"iperf -c {dstip} -b 1M -p {value} -t 15")

    else:
        stdin2, stdout2, stderr2 = p2.exec_command(f"iperf -s -u -p {value} -i 1")
        stdin1, stdout1, stderr1 = p1.exec_command(f"iperf -c {dstip} -u -b 1M -p {value} -t 15")
